...
- (void)testToJSON {
    ...
    XCTAssertTrue([[JSON class] isSubclassOfClass:[NSDictionary class]], @"Generated JSON has the wrong class");
    XCTAssertEqual([(NSDictionary *)JSON count], (NSUInteger)5, @"Initial dictionary had the wrong number of entries");
    XCTAssertEqualObjects([(NSDictionary *)JSON objectForKey:@"name"], self.summary.name, @"Wrong name.");
    XCTAssertEqualObjects([(NSDictionary *)JSON objectForKey:@"id"], self.summary.jiveId, @"Wrong id.");
    XCTAssertEqualObjects([(NSDictionary *)JSON objectForKey:@"type"], self.summary.type, @"Wrong type");
    XCTAssertEqualObjects([(NSDictionary *)JSON objectForKey:@"html"], self.summary.html, @"Wrong html");
    XCTAssertEqualObjects([(NSDictionary *)JSON objectForKey:@"uri"], self.summary.uri, @"Wrong uri");
}
...
- (void)testPlaceParsing {
    ...
    XCTAssertEqual([newSummary class], [self.summary class], @"Wrong item class");
    XCTAssertEqualObjects(newSummary.jiveId, self.summary.jiveId, @"Wrong id");
    XCTAssertEqualObjects(newSummary.name, self.summary.name, @"Wrong name");
    XCTAssertEqualObjects(newSummary.type, self.summary.type, @"Wrong type");
    XCTAssertEqualObjects(newSummary.html, self.summary.html, @"Wrong html");
    XCTAssertEqualObjects(newSummary.uri, self.summary.uri, @"Wrong uri");
}
